...
namespace OpenSooq.Areas.Dashboard.Controllers
{
    ...
    public class ProductShopsController : Controller
    {
        ...
        public ProductShopsController(AppDbContext context ,IWebHostEnvironment hostEnvironment)
        {
            _context = context;
            _hostEnvironment = hostEnvironment;
        }

        // GET: Dashboard/ProductShops
        ...
        public async Task<IActionResult> Index(string sortOrder, string currentFilter, string searchString, int? pageNumber)
        {
            ...
            var productShops = from s in _context.ProductShops.Include(u => u.Shop) select s;
            if (!String.IsNullOrEmpty(searchString))
            {
                productShops = productShops.Where(s => s.ProductName.Contains(searchString));
            }
            switch (sortOrder)
            {
                case "name_desc":
                    productShops = productShops.OrderByDescending(s => s.ProductName);
                    break;
                case "Date":
                    productShops = productShops.OrderBy(s => s.CreationDate);
                    break;
                case "date_desc":
                    productShops = productShops.OrderByDescending(s => s.CreationDate);
                    break;
                default:
                    productShops = productShops.OrderBy(s => s.ProductName);
                    break;
            }

            int pageSize = 4;
            return View(await PaginatedList<ProductShop>.CreateAsync(productShops.AsNoTracking(), pageNumber ?? 1, pageSize));
        }
        ...
        public async Task<IActionResult> Create([Bind("ProductShopId,ProductName,ProductDesc,ImageFile,ProductImg,Price,ShopId,CreationDate,IsDeleted,IsActive")] ProductShop productShop)
        {
            ...
            ViewData["ShopId"] = new SelectList(_context.Shops, "ShopId", "ShopName", productShop.ShopId);
            return View(productShop);
        }
